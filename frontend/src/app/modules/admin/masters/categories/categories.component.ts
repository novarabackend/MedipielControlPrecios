...
export class CategoriesComponent {
    ...
    load(): void {
        ...
        this._service.getCategories().subscribe({
            ...
            error: () => {
                this.error.set('No se pudo cargar Categorias.');
                this.loading.set(false);
            },
        });
    }
    ...
    create(): void {
        ...
        this._service.createCategory(name).subscribe({
            next: () => {
    ...
    saveEdit(): void {
        ...
        this._service.updateCategory(target.id, name).subscribe({
            next: () => {
                this.cancelEdit();
                this.load();
            },
        ...
        reader.onload = () => {
            try {
                ...
                from(uniqueNames)
                    .pipe(
                        concatMap((name) =>
                            this._service.createCategory(name).pipe(
                                catchError((error) => {
                                    if (error?.status === 409) {
                                        summary.skipped += 1;
                                    } else {
    ...
    downloadTemplate(): void {
        ...
        anchor.href = url;
        anchor.download = 'plantilla_categorias.xlsx';
        anchor.click();
